-- ============================================
-- TeachBuddy.ai - Supabase Database Schema
-- ============================================
...
CREATE TABLE questions (
  ...
  options JSONB, -- For MC: ["option1", "option2", ...] | For T/F: null
...
-- 6. Test assignments (teacher assigns test to student)
...
-- 7. Test attempts (student takes a test)
...
-- 8. Student responses (per question per attempt)
...
CREATE INDEX idx_tests_material ON tests(material_id);
...
CREATE INDEX idx_attempts_assignment ON test_attempts(assignment_id);
...
CREATE INDEX idx_tutoring_attempt ON tutoring_sessions(attempt_id);
...
-- Materials: teachers can CRUD their own, students can read assigned
...
CREATE POLICY "Anyone can view published tests by code" ON tests
  FOR SELECT USING (is_published = true AND access_code IS NOT NULL);

-- Questions: teachers can manage questions of their tests, students can read for assigned tests
...
-- Test assignments: teachers can manage, students can view their own
...
CREATE POLICY "Students can join tests by code" ON test_assignments
  FOR INSERT WITH CHECK (auth.uid() = student_id);

-- Test attempts: students can manage their own, teachers can view for their tests
...
-- Student responses: students can manage their own, teachers can view
...
-- Tutoring sessions: students can manage their own, teachers can view
